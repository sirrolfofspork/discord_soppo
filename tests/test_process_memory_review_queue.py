...
class ProcessMemoryReviewQueueGuardTests(unittest.TestCase):
    def test_queue_status_counts(self):
        from tools.process_memory_review_queue import queue_status_counts
        ...
        self.assertEqual(counts, {"approved": 1, "pending": 2})
    ...
    def test_apply_allowed_with_hot_even_when_service_active(self):
        from tools.process_memory_review_queue import assert_safe_to_apply_memories

        self.assertIsNone(
            assert_safe_to_apply_memories(force=False, hot=True, is_active_runner=lambda: "active")
        )
